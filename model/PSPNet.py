...
class conv2DBatchNormRelu(nn.Module):
    ...
    # pytorch踩坑記錄-Error: Expected more than 1 value per channel when training
    # https://blog.csdn.net/lrs1353281004/article/details/108262018
    def forward(self,x):
        x=self.conv(x)
        x=self.batchnorm(x) 
        outputs=self.relu(x)
...
class PSPNet(nn.Module):
    def __init__(self, n_classes, PSP_h, PSP_w, Auxiliray = False):
        ...
        self.pyramid_pooling = PyramidPooling(in_channels=2048, pool_sizes=[6, 3, 2, 1], height= int((PSP_h/div_num)), width=int((PSP_w/div_num)))
        
        # decode module
        self.decode_feature = DecodePSPFeature(height=PSP_h, width=PSP_w, n_classes=n_classes)
        
        # auxloss module
        self.aux = AuxilirayPSPLayers(in_channels=1024, height=PSP_h, width=PSP_w, n_classes=n_classes)
...
if __name__ == '__main__':   
    model = PSPNet(n_classes = 5, PSP_h = 256, PSP_w=64)
    print(model)    