...
class COCOSegmentation(Dataset):
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        # coco.showAnns(cocotarget)
        # plt.show()
        
        # _target = Image.fromarray(self._gen_seg_mask(
        #     cocotarget, img_metadata['height'], img_metadata['width']))
        if self.year == '2014':
            target_path = os.path.join(self.mask_folder, '{}{}'.format(self.split, self.year) + path.replace("jpg", "png"))
        else:
            target_path = os.path.join(self.mask_folder, path.replace("jpg", "png"))
        _target = Image.open(target_path).convert('P')
...
if __name__ == "__main__":
    # from dataset import custom_transforms as tr
    from dataset.utils import decode_segmap
