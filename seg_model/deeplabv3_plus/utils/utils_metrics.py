...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):  
    ...
    #------------------------------------------------#
    gt_imgs     = [join(gt_dir, x.split()[1] + ".png") for x in png_name_list]  
    pred_imgs   = [join(pred_dir, os.path.basename(x.split()[1]) + ".png") for x in png_name_list]
    #------------------------------------------------#
    #   读取每一个（图片-标签）对
    #------------------------------------------------#
    ...
    PA_Recall   = per_class_PA_Recall(hist) # physical activity (PA)
