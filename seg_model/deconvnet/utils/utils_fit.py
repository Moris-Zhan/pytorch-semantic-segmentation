...
from seg_model.deconvnet.nets.deconvnet_training import CE_Loss, Dice_loss, Focal_Loss
from seg_model.deconvnet.utils.utils import get_lr
from seg_model.deconvnet.utils.utils_metrics import f_score
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            pbar.set_postfix(**{'total_loss': total_loss / (iteration + 1), 
                                ...
                                'lr'        : get_lr(optimizer)})
            pbar.update(1)
            loss_history.step(total_loss / (iteration + 1), total_f_score / (iteration + 1), (epoch_step * epoch + iteration))
...
def fit_one_epoch_no_val(model_train, model, loss_history, optimizer, epoch, epoch_step, gen, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes):
    ...
    loss_history.epoch_loss_no_val(total_loss/(epoch_step+1), epoch+1)
    print('Finish Validation')
    print('Epoch:'+ str(epoch+1) + '/' + str(Epoch))
    print('Total Loss: %.3f' % (total_loss / (epoch_step + 1)))
    torch.save(model.state_dict(), '%s/ep%03d-loss%.3f.pth'%(loss_history.log_dir, (epoch + 1), total_loss / (epoch_step + 1)))
