...
def get_opts(Train=True):
    ...
    #                   x : 对应yolov7_x
    #############################################################################################    
    opt.net = 'deconvnet'     # [unet, pspnet, segnet, fcn, deconvnet, fpn, deeplab_v3, deeplab_v3_plus, segformer]
    opt.model_path      = '' #coco
    opt.input_shape     = [448, 448]  
    opt.pretrained      = True
    ...
    if Train:
        ...
        from seg_model.deconvnet.deconvnet import DeconvNet
        opt.Model_Pred = DeconvNet
    return opt
